...
-> ![目录结构](./directory_tree.jpg =250x400) <-
...
可以看出朴素的O(n^2)算法的时间随着N确实以平方规律增长, 十分之快(由于O(n^2)算法已经太慢了，所以没有继续做更多点的对比实验了)。相比其来说, 分治算法十分之快, 其能在几秒内完成1000000个点的最近点对计算。单独画出分治算法更多点(从100k ~ 1000k)的测试数据如下:
